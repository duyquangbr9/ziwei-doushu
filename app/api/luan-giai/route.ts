...
export async function POST(req: NextRequest) {
  ...
  // Build mô tả lá số để gửi cho Gemini
  const palaceDesc = palaces.map((p: any) => {
    ...
    const dxAge = p.daXianAge ? ` | Đại hạn: ${p.daXianAge[0]}-${p.daXianAge[1]}` : '';
    return `- Cung ${tenCung} (${BRANCHES[p.branch]})${dxAge}: ${sao}`;
  }).join('\n');

  const prompt = `Bạn là chuyên gia Tử Vi Đẩu Số. Hãy luận giải lá số tử vi sau bằng tiếng Việt, ngắn gọn súc tích, chia thành các phần rõ ràng.
...
Hãy luận giải theo cấu trúc JSON sau (chỉ trả về JSON, không có text thừa):
{
  "tongQuat": "Nhận xét tổng quát về lá số, tính cách, bản mệnh (3-4 câu)",
  "tinhDuyen": "Luận giải về tình duyên hôn nhân dựa trên cung Phu Thê và các sao liên quan (2-3 câu)",
  "suNghiep": "Luận giải về sự nghiệp công danh dựa trên cung Quan Lộc (2-3 câu)",
  "taiLoc": "Luận giải về tài lộc dựa trên cung Tài Bạch (2-3 câu)",
  "daiHan": "Luận giải về đại hạn hiện tại và vận hạn gần nhất (2-3 câu)",
  "cacCung": {
    "Mệnh": "luận ngắn gọn 1-2 câu",
    "Phu Thê": "...",
    "Quan Lộc": "...",
    "Tài Bạch": "...",
    "Phúc Đức": "...",
